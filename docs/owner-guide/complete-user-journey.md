...
```mermaid
flowchart TD
    A[Customer Visits XINVORA] --> B[Homepage & Hero Banner]
    B --> C{Browsing Option}
    C -->|Collections| D[Collection / Category Page]
    C -->|Shop the Look| E[Lookbook & Matching Outfits]
    C -->|Search| F[Search Bar]
    
    D --> G[Product Detail Page]
    E --> G
    F --> G
    
    G --> H[Select Color & Size]
    H --> I[Add to Cart]
    I --> J[Review Cart Drawer / Page]
    J --> K[Proceed to Checkout]
    K --> L[Enter Shipping Address & Payment]
    L --> M[Order Placed & Confirmation]
```
